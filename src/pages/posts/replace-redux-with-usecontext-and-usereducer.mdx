...
Everytime `Counter.Provider` rerenders (by another state or something like that), it will create a new `value` object. That means context changes and it will lead to rerender every child component subscribing context.
