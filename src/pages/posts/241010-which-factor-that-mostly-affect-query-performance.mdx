...
author: sonht1109
date: 2024-10-10
description: People tend to believe that, the query is slow just because the table has lots of rows and all they need is indexing. Is that always true? Or which factors that mostly decide how fast the query is?
tag:
  - database
keywords:
  - database
...
People tend to believe that, the query is slow just because the table has lots of rows and all they need is indexing. Is that always true? Or which factors that mostly decide how fast the query is?
...
## Number of total rows is "not" the X factor

I will prove that the query is even slow on 0-row table.
...
Come back to our demo and compare the total scanned pages to see the difference on performance.
