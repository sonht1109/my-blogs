...
- (2) `Auth Service` does its stuff then pushes message to queue. The messasge will be like `{"user_id": <user_id>}`.
- (3) The message then will be consumed by `Voucher Service` to distribute the voucher to user.
- (4) Finally, send notification to user to tell they that their voucher is ready to use.

So the input is `user_id`, output is voucher item distributed to that user.

Let's see how `Voucher Service` distribute voucher item to user. In real world, **this service is so complicated that none of us really knows how it works** hahaha, but just imagine that we have one table called `voucher_item`.
...
If `user_id` is not null (eg: 1), it means `user 1` has voucher item, in this case `user 1` has `voucher item 1`. Otherwise, the voucher is waiting to claim.
...
Actually, they are quite the same, but the input now will be array of users instead of one user.

But, as I said, the `Voucher Service` is too complicated, it will not be smart to implement another function to handle that. The easiest way that I come up with is, we will push a bunch of message into queue.
...
- 2nd message is consumed right after that, it gets `voucher_item 1` database too since 1st message is still in progress.
...
Okay, now our issue is, 2 processes both get access to the same resource. Definitely, **distributed lock** will save us.

Actually, I intended to implement distributed lock by myself using **Redis** but then I was recommended using [**Redlock**](https://redis.io/docs/latest/develop/use/patterns/distributed-locks/) which works perfectly with multiple Redis nodes.

Let's apply lock into `Voucher Service`. But where should I put the lock? It should be after getting voucher item, since I can use `voucher_item.id` to prevent it from being handled by multiple processes.
...
- 2nd message is consumed, it gets `voucher_item 1`. It tries to acquire the lock `voucher_item:1` but not succeed since it's being held by 1st. At this step, we can retry 2nd message so that it can be handled with another voucher item.
- After 1st message finishes, let's release the lock. It's useless now.
...
- 2nd acquires lock and then updates `voucher_item 1`.
Yahhhhhh. Reasonable. The process that acquires lock and update voucher item is too fast, it means the lock is only hold within short time. 
...
I change the lock release time from `t1` to `t2`. But you may ask, what time period (t2 - t1) is enough? I think it should be at least equal to the execution time of logic between acquiring lock and releasing lock. For example, the logic above takes 10ms to acquired lock + update voucher item + release lock, then t2 - t1 should >= 4ms.
...
## Reference
- [Redlock](https://www.youtube.com/watch?v=3uNsKNCegcM&list=LL&index=3&t=653s)